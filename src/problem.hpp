...
class Problem : public node::ObjectWrap {
 ...
 private:
    ...
    static NAN_METHOD(New) {
        ...
        GLP_CATCH_RET(Problem *obj = new Problem(); obj->Wrap(info.This()); info.GetReturnValue().Set(info.This());)
    }

    static NAN_METHOD(LoadMatrix) {
        V8CHECK(info.Length() != 4, "Wrong number of arguments");
        V8CHECK(
            !info[0]->IsInt32() || !info[1]->IsInt32Array() || !info[2]->IsInt32Array() || !info[3]->IsFloat64Array(),
            "Wrong arguments");
    ...
    static NAN_METHOD(SimplexSync) {
        ...
        GLP_CATCH_RET(glp_smcp scmp; glp_init_smcp(&scmp);
                      if (info.Length() == 1) if (!SmcpInit(&scmp, info[0])) return;

                      Problem *lp = ObjectWrap::Unwrap<Problem>(info.Holder()); V8CHECK(!lp->handle, "object deleted");
                      V8CHECK(lp->thread.load(), "an async operation is inprogress");

                      glp_simplex(lp->handle, &scmp);)
    }

    class SimplexWorker : public AsyncEventEmittingCWorker<1024> {
     public:
    ...
    static NAN_METHOD(ExactSync) {
        ...
        GLP_CATCH_RET(glp_smcp scmp; glp_init_smcp(&scmp); if (info.Length() == 1) {
            if (info[0]->IsObject())
    ...
    class ExactWorker : public Nan::AsyncWorker {
     public:
        ExactWorker(Nan::Callback *callback, Problem *lp) : Nan::AsyncWorker(callback), lp(lp) { glp_init_smcp(&smcp); }
        void WorkComplete() {
            lp->thread = false;
            Nan::AsyncWorker::WorkComplete();
        }
    ...
    static NAN_METHOD(InteriorSync) {
        ...
        GLP_CATCH_RET(glp_iptcp iptcp; glp_init_iptcp(&iptcp);
                      if (info.Length() == 1) if (!IptcpInit(&iptcp, info[0])) return;
    ...
    class InteriorWorker : public Nan::AsyncWorker {
     public:
        InteriorWorker(Nan::Callback *callback, Problem *lp) : Nan::AsyncWorker(callback), lp(lp) {
    ...
    static NAN_METHOD(ReadMpsSync) {
        ...
        V8CHECK(!info[0]->IsInt32() || !(info[1]->IsObject() || info[1]->IsNull() || info[1]->IsUndefined()) ||
                    !info[2]->IsString(),
                "Wrong arguments");

        GLP_CATCH_RET(glp_mpscp mpscp; glp_init_mpscp(&mpscp);
                      if (info.Length() == 1) if (!MpscpInit(&mpscp, info[0])) return;

                      Problem *lp = ObjectWrap::Unwrap<Problem>(info.Holder()); V8CHECK(!lp->handle, "object deleted");
                      V8CHECK(lp->thread.load(), "an async operation is inprogress");

                      int ret = glp_read_mps(lp->handle, info[0]->Int32Value(), &mpscp, V8TOCSTRING(info[2]));
                      if (mpscp.obj_name) delete[] mpscp.obj_name; info.GetReturnValue().Set(ret);)
    }
    ...
    static NAN_METHOD(ReadMps) {
        V8CHECK(info.Length() != 4, "Wrong number of arguments");
        V8CHECK(!info[0]->IsInt32() || !(info[1]->IsObject() || info[1]->IsNull()) || !info[2]->IsString() ||
                    !info[3]->IsFunction(),
                "Wrong arguments");
    ...
    static NAN_METHOD(WriteMpsSync) {
        ...
        GLP_CATCH_RET(
            glp_mpscp mpscp; glp_init_mpscp(&mpscp); if (info.Length() == 1) if (!MpscpInit(&mpscp, info[0])) return;
    ...
    static NAN_METHOD(IntoptSync) {
        ...
        GLP_CATCH_RET(glp_iocp iocp; glp_init_iocp(&iocp);
                      if (info.Length() == 1) if (!IocpInit(&iocp, info[0])) return;

                      Problem *lp = ObjectWrap::Unwrap<Problem>(info.Holder()); V8CHECK(!lp->handle, "object deleted");
                      V8CHECK(lp->thread.load(), "an async operation is inprogress");

                      glp_intopt(lp->handle, &iocp); if (iocp.cb_info) delete (Nan::Callback *)iocp.cb_info;
                      if (iocp.save_sol) delete[] iocp.save_sol;)
    }

    class IntoptWorker : public Nan::AsyncWorker {
     public:
        IntoptWorker(Nan::Callback *callback, Problem *lp) : Nan::AsyncWorker(callback), lp(lp) {
            glp_init_iocp(&parm);
    ...
    static NAN_METHOD(CheckKkt) {
        ...
        int ae_ind, re_ind;
        GLP_CATCH_RET(glp_check_kkt(lp->handle, info[0]->Int32Value(), info[1]->Int32Value(), &ae_max, &ae_ind, &re_max,
                                    &re_ind);)

        Nan::Callback *cb = new Nan::Callback(Local<Function>::Cast(info[2]));
        const unsigned argc = 4;
        Local<Value> argv[argc] = {Nan::New<Number>(ae_max), Nan::New<Int32>(ae_ind), Nan::New<Number>(re_max),
    ...
    static NAN_METHOD(PrintRangesSync) {
        V8CHECK(info.Length() != 3, "Wrong number of arguments");
        V8CHECK(!(info[0]->IsInt32Array() || info[0]->IsNull() || info[0]->IsUndefined()) || !info[1]->IsInt32() ||
                    !info[2]->IsString(),
                "Wrong arguments");
        ...
        int *plist = NULL;
        int ret = 0;
        GLP_CATCH(if (info[0]->IsInt32Array()) {
    ...
    static NAN_METHOD(PrintRanges) {
        ...
        Nan::Callback *callback = new Nan::Callback(info[3].As<Function>());
        PrintRangesWorker *worker =
            new PrintRangesWorker(callback, lp, len, info[1]->Int32Value(), V8TOCSTRING(info[2]));
        if (len > 0) {
    ...
    static NAN_METHOD(GetBfcp) {
        ...
        GLP_CATCH_RET(glp_bfcp bfcp; glp_get_bfcp(lp->handle, &bfcp); Local<Object> ret = Nan::New<Object>();
                      GLP_SET_FIELD_INT32(ret, "type", bfcp.type); GLP_SET_FIELD_DOUBLE(ret, "pivTol", bfcp.piv_tol);
                      GLP_SET_FIELD_INT32(ret, "pivLim", bfcp.piv_lim); GLP_SET_FIELD_INT32(ret, "suhl", bfcp.suhl);
    ...
    static NAN_METHOD(SetBfcp) {
        ...
        GLP_CATCH_RET(glp_bfcp bfcp; glp_get_bfcp(lp->handle, &bfcp);
    ...
    class FactorizeWorker : public Nan::AsyncWorker {
     public:
        FactorizeWorker(Nan::Callback *callback, Problem *lp) : Nan::AsyncWorker(callback), lp(lp) {}
        void WorkComplete() {
            lp->thread = false;
            Nan::AsyncWorker::WorkComplete();
        }
